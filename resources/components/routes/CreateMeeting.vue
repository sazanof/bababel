        ...
        >
          ...
          >
            ...
            <VDateInput
              ...
              variant="outlined"
              :min-date="new Date()"
              @update:model-value="updateDateTime($event, time)"
            >
              <template #append-inner>
                <TimePicker
                  v-model="time"
                  @update:model-value.passive="updateDateTime(date,$event)"
                >
...
export default {
  ...
  async created() {
    ...
    if (this.meeting.id > 0) {
      await this.getMeeting().catch(e => {
        ...
        this.$store.commit('addNotification', createErrorNotification(this.$t('Error getting meeting')))
      })
  ...
  methods: {
    async saveMeeting() {
      ...
      this.meeting.date = this.datetime
    ...
    async removeDocument(id) {
      await this.$store.dispatch('removeDocument', id).then(() => {
        this.$store.commit('addNotification', createSuccessNotification(this.$t('Document deleted')))
      })
    },
    async deleteMeeting() {
      if (this.id) {
    ...
    updateDateTime(date, time) {
      if(this.date === null){
        this.datetime = m(new Date()).format('YYYY-MM-DD')
      } else {
        this.datetime = m(date).format('YYYY-MM-DD')
      }
      if(time === null) {
        time = '00:00'
      }
      this.datetime = `${this.datetime} ${time}`
